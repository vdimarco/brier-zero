...
export async function fetchMatches({ force = false } = {}) {
  ...
  // limit: ESPN caps a date-range scoreboard at 100 events by default, which
  // silently drops the LAST fixtures of a 104-match tournament — the
  // semifinals and final. An explicit high limit returns the whole bracket.
  const data = await fetchScoreboard(`${SCOREBOARD_URL}?dates=${key}&limit=500`);
