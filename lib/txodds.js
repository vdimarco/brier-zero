...
export async function fetchFixtures({ force = false, sinceEpochDay = null } = {}) {
  // Window: from a week back so recently finished fixtures stay matchable;
  // callers pricing historical fixtures pass an earlier sinceEpochDay. A
  // cached list is reused only when it covers the requested window.
  const start = sinceEpochDay ?? epochDay(new Date(Date.now() - 7 * EPOCH_DAY_MS));
  if (
    !force && fixturesCache.list &&
    Date.now() - fixturesCache.at < FIXTURES_TTL_MS &&
    fixturesCache.start <= start
  ) {
  ...
  fixturesCache = { at: Date.now(), start, list };
  return list;
}
...
export async function marketProbs(match, { asOf = null } = {}) {
  try {
    // Historical pricing must see fixtures from around the asOf date, not
    // just the default recent window.
    const fixtures = await fetchFixtures(
      asOf ? { sinceEpochDay: epochDay(new Date(asOf)) - 1 } : {}
    );
    const found = matchFixture(match, fixtures);
    if (!found) return null;
    // A snapshot at exactly asOf can land in a window with no pre-match
    // 1X2 message; step back a little before giving up.
    let snap = null;
    for (const backoffMs of [0, 10 * 60000, 2 * 3600000]) {
      snap = await fetchOddsSnapshot(found.fixture.fixtureId, {
        asOf: asOf ? new Date(asOf).getTime() - backoffMs : null,
      });
      if (snap?.prices || !asOf) break;
    }
...
function u32le(n) {
  ...
  b.writeUInt32LE(n >>> 0, 0); // stat fields observed on devnet are small
  return b;                    // non-negative counters (goals, cards, ...)
}

// The devnet leaf for a single stat: sha256(u32le(key) || u32le(value) ||
// u32le(period)). Reverse-engineered from the captured fixture — the
// on-chain program is Rust/Anchor, so there is no JS reference for this;
// confirmed by folding it through statProof and landing exactly on the
// payload's own `eventStatRoot`.
function statLeaf(stat) {
  return sha256(Buffer.concat([u32le(stat.key), u32le(stat.value), u32le(stat.period)]));
}

// Fold a Merkle path. Each node is { hash: number[32], isRightSibling }
// (the field names/shapes TxLINE actually returns — not the hex-string
// `{ hash, right }` shape sketched in the docs' generic pseudocode).
// isRightSibling: true means the sibling sits to the right of the
// running accumulator, i.e. hash(acc || sibling); false means
// hash(sibling || acc).
function foldPath(leaf, nodes) {
  let acc = leaf;
  for (const n of nodes ?? []) {
    const sibling = Buffer.from(n.hash);
    acc = n.isRightSibling
      ? sha256(Buffer.concat([acc, sibling]))
      : sha256(Buffer.concat([sibling, acc]));
  }
  return acc;
}

// Recompute the committed daily root from the proof payload alone (no
// I/O): stat leaf -> statProof -> eventStatRoot -> subTreeProof ->
// eventStatsSubTreeRoot -> mainTreeProof -> daily main root. `eventRoot`
// and `subRoot` are exposed alongside `root` because the payload itself
// carries their expected values (`eventStatRoot`, `summary.
// eventStatsSubTreeRoot`), which is what makes this self-verifiable
// without a live chain read; only the final `root` needs the on-chain
// `daily_scores_roots` account (see scripts/verify-results.js) to confirm.
export function recomputeRoot(payload) {
  const leaf = statLeaf(payload.statToProve);
  ...
  return {
    ...
    statValue: payload.statToProve,
    eventRoot: eventRoot.toString('hex'),
    subRoot: subRoot.toString('hex'),
  };
}
