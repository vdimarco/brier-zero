...
import { marketProbs, txoddsReady } from './txodds.js';
...
// Every entrant that has ever competed: the active roster plus retired
// entries — models substituted out mid-tournament. Retired entrants never
// collect new forecasts, but their records stay on the leaderboard and
// their stored forecasts keep rendering.
export function loadEntrants() {
  const file = path.join(root, 'models.config.json');
  const cfg = JSON.parse(fs.readFileSync(file, 'utf8'));
  return [...cfg.models, ...(cfg.retired ?? [])];
}

// Geopolitical blocs for the by-country view ({ us: { label, emoji }, ... }).
export function loadBlocs() {
  const file = path.join(root, 'models.config.json');
  const cfg = JSON.parse(fs.readFileSync(file, 'utf8'));
  return cfg.blocs ?? {};
}
...
export function predictorReady() {
  return DEMO || Boolean(process.env.OPENROUTER_API_KEY) || txoddsReady();
}

// The market competitor (provider: "txodds" in models.config.json) is not
// an LLM: its "forecast" is the de-vigged TxODDS StablePrice consensus.
export function isMarketModel(model) {
  return model?.provider === 'txodds';
}

// One forecast from the market: de-vigged odds in the match's own market.
// Null when odds are not available yet — the collector just retries later,
// exactly like a transient LLM failure, but without storing an error.
async function marketForecast(model, match, opts = {}) {
  if (DEMO) {
    // demoPredict's hash occasionally lands outside the valid simplex and
    // yields null; salt the seed until it sticks (deterministic per match).
    let parsed = null;
    for (let salt = 0; salt < 8 && !parsed?.probs; salt++) {
      parsed = demoPredict(`${model.id}#${salt}`, match);
    }
    return { ...parsed, rationale: 'Demo mode: placeholder odds, not real TxODDS prices.' };
  }
  if (!txoddsReady()) return null;
  return marketProbs(match, opts);
}
...
export function demoPredict(modelId, match) {
...
export async function callOpenRouter(modelId, prompt) {
...
export async function predictOne(model, match) {
  ...
  // No OpenRouter key: LLM models are silently skipped (retried once a key
  // appears) instead of writing error records every collection cycle. The
  // market competitor keeps collecting on its own TXODDS credentials.
  if (!DEMO && !isMarketModel(model) && !process.env.OPENROUTER_API_KEY) {
    return { model: model.id, status: 'skipped-no-key' };
  }
  ...
  try {
    ...
    if (isMarketModel(model)) {
      parsed = await marketForecast(model, match);
      if (!parsed) return { model: model.id, status: 'skipped-no-odds' };
    } else if (DEMO) {
  ...
  for (const match of matches) {
    const settled = await Promise.allSettled(
      models.map(async (model) => {
        ...
        let parsed;
        if (isMarketModel(model)) {
          // Retro market line: the odds snapshot as of kickoff, so a
          // backfilled market record prices the same moment the retro LLM
          // prompt describes. Skipped when historical odds are missing.
          parsed = await marketForecast(model, match, { asOf: match.kickoff });
          if (!parsed) return { model: model.id, status: 'skipped-no-odds' };
        } else if (DEMO) {
          parsed = demoPredict(model.id, match);
        } else {
...
export async function collectOutright(teams, models = loadModels(), opts = {}) {
  ...
  // The market has no view on "who lifts the trophy" until a to-win
  // outright offer is mapped, so it sits out the trophy race rounds.
  const roster = models.filter((m) => !isMarketModel(m));
  const settled = await Promise.allSettled(
    roster.map(async (model) => {
      if (DEMO) {
  ...
  return { ok, failed: roster.length - ok };
}
...
export async function snapshotMatches(liveMatches, models = loadModels()) {
  ...
  for (const match of liveMatches) {
    const settled = await Promise.allSettled(
      models.map(async (model) => {
        if (isMarketModel(model)) {
          // Live market view: the CURRENT StablePrice line, repricing on
          // the same events the models react to.
          const parsed = await marketForecast(model, match);
          if (!parsed) throw new Error('no live odds for fixture');
          return { model: model.id, ...parsed };
        }
