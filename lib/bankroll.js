...
//
// Odds source, per market:
// - regulation (group stage / 3rd place): the RAW 1X2 decimal line recorded
//   in the market entry's rationale at collection time ("decimal H/D/A" or
...
// - advance (knockouts): bookmakers don't quote a single "advances" price in
//   our feed; the stored market forecast is the de-vigged two-way split. Bets
//   settle at fair odds d = 1/q. Disclosed in the UI. oddsType: 'fair'.
//
// Rules (fixed):
//   edge_o      = p_o * d_o - 1
...
export function lineFor(match, marketPred) {
...
export function computeBankrolls(matches, predictions, entrants) {
  ...
  for (const match of settled) {
    ...
    if (!outcome) continue;
    const line = lineFor(match, preds[MARKET_ID]);
    if (!line) continue; // no locked line → nobody can bet this match

    for (const entrant of entrants) {
      const modelId = entrant.id;
      if (modelId === MARKET_ID) continue; // can't bet against itself
      const pred = preds[modelId];
      // Same lock discipline as scoring: an eligible pre-kickoff forecast in
      // this match's own market. Anything else simply isn't a priceable bet.
      if (!pred?.probs || !pred.eligible || !fixtureMatches(pred, match)) continue;
      if (predictionMarket(pred) !== market) continue;
